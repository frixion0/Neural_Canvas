...
function playErrorSound() {
    playSound(errorBuzzBuffer);
}

function playButtonClickSound() {
    playSound(tabClickBuffer);
}
...
// Data structure for image models
const imageModels = [
...
const elements = {
    ...
    imageModelSelector: document.getElementById('imageModelSelector'), // NEW: custom model selector button
    selectedImageModelText: document.getElementById('selectedImageModelText'), // NEW: text to display selected model
    ...
    // NEW: Model selection modal elements
    modelSelectionModal: document.getElementById('modelSelectionModal'),
    ...
    confirmModelSelection: document.getElementById('confirmModelSelection'),
};

// Initialize selected image model text display
elements.selectedImageModelText.textContent = imageModels.find(m => m.id === currentSelectedImageModel)?.name || currentSelectedImageModel;
...
originalTabButtons.forEach((btn, index) => {
    if (index < tabs.length) {
        btn.addEventListener('click', () => {
            playButtonClickSound(); // Play sound on tab click
...
// Image model selection modal functionality
let tempSelectedModelId = currentSelectedImageModel; // Temporary storage for modal selection

elements.imageModelSelector.addEventListener('click', () => {
    playButtonClickSound(); // Add sound
    populateModelModal();
    elements.modelSelectionModal.style.display = 'flex';
});

elements.closeModalBtn.addEventListener('click', () => {
    playButtonClickSound(); // Add sound
    elements.modelSelectionModal.style.display = 'none';
});

elements.confirmModelSelection.addEventListener('click', () => {
    playButtonClickSound(); // Add sound
...
function populateModelModal() {
    elements.modelCardsContainer.innerHTML = ''; // Clear previous cards
    imageModels.forEach(model => {
        const modelCard = document.createElement('div');
        modelCard.className = 'model-card';
        modelCard.dataset.modelId = model.id;
        
        // Add disabled class for Imagen models
        const isDisabled = model.id === 'provider-4/imagen-3' || model.id === 'provider-4/imagen-4';
        if (isDisabled) {
            modelCard.classList.add('disabled-model-card');
        }

        if (model.id === tempSelectedModelId) {
            modelCard.classList.add('selected');
        }
        if (model.isUncen) {
            modelCard.classList.add('model-card-highlight');
        }
        ...
        modelCard.addEventListener('click', () => {
            ...
            if (modelCard.classList.contains('disabled-model-card')) {
                playErrorSound(); 
                displayStatusMessage('globalAppStatus', 'error', 'This model is currently unavailable for selection.', 2000);
                return;
            }
        ...
        elements.modelCardsContainer.appendChild(modelCard);
    });
}
...
elements.generateImageButton.addEventListener('click', async () => {
    ...
    if (!prompt) {
        ...
        playErrorSound(); // Add error sound for empty prompt
        return;
    }
    ...
    // Disable style and model selector
    elements.imageStyle.disabled = true;
    elements.imageModelSelector.classList.add('disabled');
    elements.imageModelSelector.style.pointerEvents = 'none';
    ...
    try {
        ...
        playErrorSound(); // Play error sound on generation failure
    } finally {
        ...
        // Re-enable style and model selector
        elements.imageStyle.disabled = false;
        elements.imageModelSelector.classList.remove('disabled');
        elements.imageModelSelector.style.pointerEvents = 'auto';
    }
});
...
elements.reEditWebsiteButton.addEventListener('click', () => {
    playButtonClickSound(); // Add sound
    reEditWebsite();
});
...
function reEditWebsite() {
    if (!currentWebsiteCode) {
        ...
        playErrorSound(); // Add error sound
        return;
    }
...
elements.removeChatImageBtn.addEventListener('click', () => {
    playButtonClickSound(); // Add sound
    removeChatImage();
});
...
function createEnhancer(promptElementId, statusElementId) {
    return async function() {
        playButtonClickSound(); // Add sound here
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Load sound files
    try {
        errorBuzzBuffer = await loadSound('error_buzz.mp3');
        tabClickBuffer = await loadSound('tab_click.mp3');
    } catch (error) {
