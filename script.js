...
// Update tab switching to include builder
const tabButtons = document.querySelectorAll('.tab-btn');
tabButtons.forEach((btn, index) => {
    btn.addEventListener('click', () => {
        const tabs = ['imageEditor', 'imageGenerator', 'videoGenerator', 'builder'];
...
// Update switchTab function to handle 4 tabs
function switchTab(tabName) {
    currentTab = tabName;
    
    // Update active states
    tabButtons.forEach((btn, index) => {
        btn.classList.toggle('active', ['imageEditor', 'imageGenerator', 'videoGenerator', 'builder'][index] === tabName);
    });
...
elements.buildWebsiteButton.addEventListener('click', async () => {
    ...
    try {
        ...
        // Store the code for download/editing
