...
async function sendBotMessage() {
    const message = elements.botInput.value.trim();
    if (!message) {
        displayStatusMessage('botStatus', 'error', 'Please enter a message.');
        return;
    }
    ...
    try {
        const response = await botClient.chat.completions.create({
            ...
            messages: [
                {"role": "system", "content": "You are a helpful AI assistant. Be concise, friendly, and provide useful responses."},
